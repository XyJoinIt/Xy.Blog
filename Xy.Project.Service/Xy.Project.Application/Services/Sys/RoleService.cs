...
namespace Xy.Project.Application.Services.Sys
{
    internal class RoleService : IRoleContract
    {
        ...
        public async Task<AppResult> AddAsync(AddRoleInputDto dto)
        {
            ...
            var role = ObjectMap.MapTo<SysRole>(dto);
        ...
        public async Task<AppResult> PageAsync(PageParam page)
        {
            ...
            page.AddOrderCondition(new OrderCondition<SysRole>(o => o.Id, OrderDirection.Ascending));
            var exp = FilterBuilder.GetExpression<SysRole>(page.FilterGroup);
            var pageList = await _roleManager.Roles.AsNoTracking()
                .Where(exp)
                .ToPageAsync<SysRole, RoleOutputPageListDto>(page.PageCondition);
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        private async Task<SysRole> FindByIdAsync(long id) => await _roleManager.FindByIdAsync(id.ToString());
    }
}
