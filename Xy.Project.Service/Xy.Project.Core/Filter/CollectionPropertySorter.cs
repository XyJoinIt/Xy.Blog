...
namespace Xy.Project.Core.Filter
{
    public static class CollectionPropertySorter<T>
    {
        ...
        /// <returns></returns>
        public static IOrderedQueryable<T> OrderBy(IQueryable<T> source, string propertyName, OrderDirection sortDirection)
        {
            propertyName.NotNullOrEmpty("propertyName");
            dynamic keySelector = GetKeySelector(propertyName);
            return sortDirection == OrderDirection.Ascending
        ...
        /// <param name="sortDirection">排序方向</param>
        /// <returns></returns>
        public static IOrderedQueryable<T> ThenBy(IOrderedQueryable<T> source, string propertyName, OrderDirection sortDirection)
        {
            propertyName.NotNullOrEmpty("propertyName");
            dynamic keySelector = GetKeySelector(propertyName);
            return sortDirection == OrderDirection.Ascending
